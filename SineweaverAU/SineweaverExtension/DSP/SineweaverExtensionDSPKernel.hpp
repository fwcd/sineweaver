//
//  SineweaverExtensionDSPKernel.hpp
//  Sineweaver
//
//  Created on 04.03.25
//
...
/*
 SineweaverExtensionDSPKernel
 As a non-ObjC class, this is safe to use from render thread.
 */
class SineweaverExtensionDSPKernel {
public:
    ...
    // Add a case for each parameter in SineweaverExtensionParameterAddresses.h
    void setParameter(AUParameterAddress address, AUValue value) {
        switch (address) {
            case SineweaverExtensionParameterAddress::gain:
                mGain = value;
                break;
        }
    }
    ...
    void handleMIDIEventList(AUEventSampleTime now, AUMIDIEventList const* midiEvent) {
        auto visitor = [] (void* context, MIDITimeStamp timeStamp, MIDIUniversalMessage message) {
            auto thisObject = static_cast<SineweaverExtensionDSPKernel *>(context);
